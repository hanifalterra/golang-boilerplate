...
func (r *productBillerRepository) getBaseQuery(filters map[string]interface{}) (string, []interface{}) {
	...
	if productID, ok := filters["product_id"].(uint); ok {
		processedFilters["product_id"] = utils_db.Filter{Operator: "=", Value: productID}
	}
	if billerID, ok := filters["biller_id"].(uint); ok {
		processedFilters["biller_id"] = utils_db.Filter{Operator: "=", Value: billerID}
	}
	if isActive, ok := filters["is_active"].(bool); ok {
		processedFilters["is_active"] = utils_db.Filter{Operator: "=", Value: isActive}
	}
